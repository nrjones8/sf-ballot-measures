...
ui <- shinyUI(
  navbarPage(
    ...
    tabPanel(
      'Overview',
      # When changes are made to the `.Rmd` file, need to re-knit those changes to the `.md` file that's loaded here
      includeMarkdown('props_overview.md')
    ),
    tabPanel(
      'Explorer',
...
server <- function(input, output) {
  # earliest_year <- input$start_year
  # latest_year <- input$end_year
  earliest_year <- 1988
  latest_year <- 2018
  ...
  # Thanks to https://stackoverflow.com/a/33500524
  # output$rmarkdown_thing <- renderUI({
  #   HTML(markdown::markdownToHTML(knit('props_overview.Rmd', quiet = TRUE)))
  # })
}
